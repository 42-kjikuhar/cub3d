...
/*   Updated: 2026/05/30 13:21:47 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player_forward(void)
{
	...
	move = dvec3_scale(MOVE_SPEED, get_player()->dir);
	move.z = 0.0;
	move_player(move);
	change_camera_pos(get_player()->pos);
}
...
void	move_player_left(void)
{
	...
	move = dvec3_scale(MOVE_SPEED, dvec3_rotate_z(get_player()->dir, -90.0));
	move.z = 0.0;
	move_player(move);
	change_camera_pos(get_player()->pos);
}
...
void	move_player_back(void)
{
	...
	move = dvec3_scale(MOVE_SPEED, dvec3_rotate_z(get_player()->dir, 180.0));
	move.z = 0.0;
	move_player(move);
	change_camera_pos(get_player()->pos);
}

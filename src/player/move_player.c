...
/*   Updated: 2026/05/27 08:57:50 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player(t_map const *map, t_player *player, t_dvec3 move);
...
void	move_player(t_map const *map, t_player *player, t_dvec3 move)
{
	...
	if (is_colliding_with_wall(map, dvec3(player->pos.x, new_pos.y, player->pos.z)))
	{
		new_pos.y = player->pos.y;
	}
	if (is_colliding_with_wall(map, dvec3(new_pos.x, player->pos.y, player->pos.z)))
	{
		new_pos.x = player->pos.x;
	}
	if (new_pos.y != player->pos.y && new_pos.x != player->pos.x \
		&& is_colliding_with_wall(map, dvec3(new_pos.x, new_pos.y, player->pos.z)))
	{
		return ;
	}
	player->pos = new_pos;

...
/*   Updated: 2026/05/28 21:14:00 by stanaka2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_mlx *mlx, t_player const *player, t_hit const *hit)
{
	...
	to_point = dvec3_sub(hit->front_pos, player->pos);
	compute_wall_point(player, to_point, &(side.top_depth), &(side.top));
	to_point = dvec3_sub(dvec3(hit->front_pos.x, hit->front_pos.y, 0.0), player->pos);
